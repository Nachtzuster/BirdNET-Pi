...
def filter_humans(detections):
    ...
    human_neighbour_mask = [False] * len(sensored_detections)
    for i, detection in enumerate(sensored_detections):
        if i != 0:
            if sensored_detections[i - 1][0][0] == 'Human_Human':
                human_neighbour_mask[i] = True
        if i != len(sensored_detections) - 1:
            if sensored_detections[i + 1][0][0] == 'Human_Human':
                human_neighbour_mask[i] = True

    clean_detections = []
    for i, (has_human_neighbour, detection) in enumerate(zip(human_neighbour_mask, sensored_detections)):
