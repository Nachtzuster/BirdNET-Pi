...
if ($image_provider && $image_provider->is_reset()) {
    $_SESSION['species_portal_v7_cache'] = [];
}
...
<div class="species-dashboard">
    ...
    <div class="species-grid">
        <?php foreach ($species_list as $bird): 
            ...
            if ($image_provider) {
                if (!isset($_SESSION['species_portal_v7_cache'])) {
                    $_SESSION['species_portal_v7_cache'] = [];
                }
                
                $search_name = trim($com_name);
                $key = array_search($search_name, array_column($_SESSION['species_portal_v7_cache'], 0));
                
                if ($key !== false) {
                    $image = $_SESSION['species_portal_v7_cache'][$key];
                    ...
                    if ($cached_image && !empty($cached_image["image_url"])) {
                        ...
                        array_push($_SESSION["species_portal_v7_cache"], $image_data);
                        $image = $image_data;
                    } else {
