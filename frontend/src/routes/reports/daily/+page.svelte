<script lang="ts">
	...
	import { detections, type DailyReport } from '$lib/api';
	...
	function syncUrl() {
		if (typeof window === 'undefined' || !currentDate) return;
	...
	async function loadReport(date?: string) {
		loading = true;
		try {
			report = await detections.dailyReport(date);
			currentDate = report.date;
			syncUrl();
		} catch (e) {
			console.error('Failed to load daily report:', e);
			toasts.show('Failed to load daily report', 'error');
		} finally {
			loading = false;
		}
	}

	function navigateDays(direction: -1 | 1) {
		if (!currentDate) return;
		const next = dateFromStr(currentDate);
		next.setDate(next.getDate() + direction);
		void loadReport(formatDate(next));
	}
...
<svelte:head>
	<title>Daily Report - BirdNET-Pi</title>
</svelte:head>

<div class="container mx-auto px-4 py-6">
	<div class="mb-6 flex flex-wrap items-start justify-between gap-3">
		<div>
			<h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Daily Report</h1>
			<p class="text-gray-600 dark:text-gray-400 mt-1">A day-by-day summary of detections, species activity, and first-time arrivals</p>
		</div>
		<a href={`/history?mode=day${currentDate ? `&date=${currentDate}` : ''}`} class="btn-secondary">Open Insights</a>
	</div>
	...
	{:else if report}
		<div class="card mb-6 overflow-hidden">
			<div class="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
				<button on:click={() => navigateDays(-1)} class="btn-ghost">Previous Day</button>
				...
				<button on:click={() => navigateDays(1)} class="btn-ghost">Next Day</button>
			</div>
		</div>
		...
		<div class="grid gap-4 mb-6 sm:grid-cols-2 lg:grid-cols-4">
			...
			<div class="card p-4">
				<p class="text-sm text-gray-500 dark:text-gray-400">Previous Day</p>
			...
			<div class="card p-4">
				<p class="text-sm text-gray-500 dark:text-gray-400">Peak Hour</p>
				<p class="text-3xl font-bold text-gray-900 dark:text-gray-100 mt-2">{hourLabel(report.peak_hour)}</p>
				<p class="text-sm mt-2 text-gray-500 dark:text-gray-400">{report.first_seen_species.length} first-seen species</p>
			</div>
		</div>
			...
			<div class="card overflow-hidden">
				...
				<div class="divide-y divide-gray-200 dark:divide-dark-border">
					{#if report.top_species.length === 0}
						<div class="px-6 py-8 text-gray-500 dark:text-gray-400">No detections were recorded during this day.</div>
					{:else}
							...
							<div class="px-6 py-4 flex items-center justify-between gap-4">
								<div class="min-w-0">
									...
									<p class={`text-sm mt-1 ${changeTone(species.change_pct)}`}>
										{#if species.is_new_this_day}
											First detected on this day
										{:else}
											{formatChange(species.change_pct)} vs previous day
										{/if}
									</p>
								</div>
			...
			<div class="card overflow-hidden">
				...
				<div class="divide-y divide-gray-200 dark:divide-dark-border">
					{#if report.first_seen_species.length === 0}
						<div class="px-6 py-8 text-gray-500 dark:text-gray-400">No new species were first detected during this day.</div>
					{:else}
